...
echo "=== INICIANDO AUTOMATIZACIÓN DE CARTOGRAPHER COMPLETO (5 EJECUCIONES) ==="

for i in {1..5}
do
    echo "--- Iniciando Ejecución $i de 5 ---"

    # 1. Arrancar Cartographer Node
    /opt/ros/jazzy/lib/cartographer_ros/cartographer_node \
        -configuration_directory /home/pharanguren/TFG/DataCartographer \
    ...
    echo "      Grabando métricas en metricas_cartographer_${i}.csv"
    echo "CPU(%),RAM(MB)" > metricas_cartographer_${i}.csv
    (
        while ps -p $NODE_PID > /dev/null; do
            RAW_DATA=$(ps -p $NODE_PID -o %cpu,rss --no-headers)
            echo $RAW_DATA | awk '{printf "%.1f,%.1f\n", $1, $2/1024}' >> metricas_cartographer_${i}.csv
            sleep 0.5
        done
    ) &
    METRICS_PID=$!
    ...
    echo "      Grabando trayectoria en bag_trayectoria_cartographer_${i}..."
    ros2 bag record -o bag_trayectoria_cartographer_${i} /tf /tf_static /odom /trajectory_node_list > /dev/null 2>&1 &
    TRAYECTORIA_PID=$!
    ...
    # 7. Guardar Mapa
    echo "      Bag finalizado. Guardando el mapa mapa_cartographer_${i}..."
    ros2 run nav2_map_server map_saver_cli -f mapa_cartographer_${i} --ros-args -p use_sim_time:=true
...
echo "=== ¡TODAS LAS EJECUCIONES DE CARTOGRAPHER COMPLETADAS! ==="