...
echo "=== INICIANDO AUTOMATIZACIÓN DE CARTOGRAPHER (Métricas Corregidas) ==="

echo "[1/5] Arrancando Cartographer..."

# 1. Llamamos directamente al binario C++ de Cartographer saltándonos "ros2 run". 
# Así nos aseguramos de que el PID ($!) es el del algoritmo real.
...
echo "      Grabando métricas del PID $NODE_PID en metricas_cartographer.csv"
echo "CPU(%),RAM(MB)" > metricas_cartographer.csv
(
    while ps -p $NODE_PID > /dev/null; do
        RAW_DATA=$(ps -p $NODE_PID -o %cpu,rss --no-headers)
        echo $RAW_DATA | awk '{printf "%.1f,%.1f\n", $1, $2/1024}' >> metricas_cartographer.csv
        sleep 0.5
    done
) &
METRICS_PID=$!
...
ros2 run nav2_map_server map_saver_cli -f mapa_cartographer
